...
旧版只删除谱号八度标记、保留 pitch 是错误的修复条件：原 OMR 已把
八度谱号作用编码进音高，删除标记后音符会改变谱面位置。
新版从原始 MXL 重新生成结果，修正谱号时按该谱号的作用范围补偿音符八度：
8vb 对应升一八度，8va 对应降一八度；下一个谱号结束旧作用范围。
按声部和谱表分别处理，包含和弦、装饰音，休止符不修改。
每个音符的八度修改都有独立 before/after 记录。
打开旧审谱状态也会从原始 MXL 重建，不在旧修复文件上叠加。
旧版“修复前后音高一致”的测试不能证明与原 PDF 一致，已被更正。
