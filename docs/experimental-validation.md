...
主按钮为“一键识别并整理”。完成后若已安装 MuseScore 会直接打开输出试听，
主页面不再前置待核对数量。需要排查时才点击“还有问题？查看检查提示”。

新增实验性自动规则：小提琴或 Audiveris 泛化为 Voice 的声部，
在系统边界反复出现交替的上下八度高音谱号，且无移调或八度线时，
自动移除八度标记，保留 pitch。原始文件保留，支持撤销。
这是启发式规则，仍可能遇到合法特殊记谱，不等于对原 PDF 的真实性证明。
真实样例自动整理 6 处、重新检查后 5 条关联提示消失。
内部按小节合并后仍有 38 个小节提示，主要是当前 OMR 的节奏识别问题；
这些不会阻断结果，也不会被假装自动修复。

同谱表的谱号可批量处理与撤销。每次修复重新审谱，已消失提示自动退出任务列表。
全谱 INFO 仅作为说明，同一小节的多个提示合并为一次检查。
...
除上面的系统性异常自动规则外，用户可在工作台逐项选择谱号修复，
